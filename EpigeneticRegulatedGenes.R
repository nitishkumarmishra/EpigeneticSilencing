...
library(pacman)
p_load(ELMER, Homo.sapiens, DMRcate, biomaRt, missMethyl, multtest, impute, matrixStats, ChAMP, TCGAbiolinks, plyr)
#################################################
...
keep <- !rowMeans(Normal.impue.BMIQ) >0.3 # >= 0.3, I am getting same number of probe
...
#####################################################
################ Function to call ENTREZ-GENE-ID and get 250bp up & downstream probes from TSS ########
...
d3$lower10SD1.282 <- -1.282*d3$hypoExpSD+d3$hypoExpMean
...
d3$corr <- cor.value
d4 <- d3[(which(d3$hyperExpMean < d3$lower10SD1.282  & d3$corr < 0)),]## Probe-gene pair, genes which have expression greater than lower 10% of Unmethylated
d5  <- d4[which(d4$percentageHyper >= 80),] #Probes which have <80% samples in Hypermethlated group have lower expression than mean of Unmethylated group
d5 <- d5[(which(d5$corr < 0)),]
#d4 <- d3[(which(d3$hyperExpMean < d3$lower10SD1.282)),]
#d5 <- d4[(which(d3$hyperExpMean < d4$lower10SD1.282  & d4$corr < 0)),]
############################################################################
#qq <- table(d4$geneID)
TotalEpiSilence <- table(d5$geneID)
TotalProbes <- table(d3$geneID)
...
#ss4 <- ifelse(ss3[,2:37]!=0, "TRUE", "FALSE")
